...
public class Lab7 extends JPanel implements GLEventListener {
	...
	private Texture textureFromResource(String resourceName) throws IOException 
	{
		...
		Texture textureFromResource;
		GLContext context = displayGL.getContext();
		context.makeCurrent();

		GL2 gl2 = context.getGL().getGL2();
		textureFromResource = AWTTextureIO.newTexture(displayGL.getGLProfile(), image, true);
		textureFromResource.setTexParameteri(gl2, GL2.GL_TEXTURE_WRAP_S, GL2.GL_REPEAT);
		textureFromResource.setTexParameteri(gl2, GL2.GL_TEXTURE_WRAP_T, GL2.GL_REPEAT);
	...
	private Texture textureFromPainting() {
		...
		Texture textureFromPainting;
		BufferedImage image = paintPanel.copyOSC();
		GLContext context = displayGL.getContext();
		ImageUtil.flipImageVertically(image); //!
		context.makeCurrent();
		GL2 gl2 = context.getGL().getGL2();
		textureFromPainting = AWTTextureIO.newTexture(displayGL.getGLProfile(), image, true);
		textureFromPainting.setTexParameteri(gl2, GL2.GL_TEXTURE_WRAP_S, GL2.GL_REPEAT);
		textureFromPainting.setTexParameteri(gl2, GL2.GL_TEXTURE_WRAP_T, GL2.GL_REPEAT);
		context.release();
		
		return textureFromPainting;
	}
	/**
	...
	public void display(GLAutoDrawable drawable) {    
		...
		{
			currentTexture.enable(gl2);	//włącza funkcję tekstury dla kontekstu OpenGL
			currentTexture.bind(gl2);	//przypisuje konkretną teksturę do bieżącego kontekstu OpenGL
