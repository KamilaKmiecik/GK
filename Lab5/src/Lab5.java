...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	private void draw(GL2 gl2) {
        ...
        switch (objectNumber) {
        case 1:
        	drawSpinner(gl2, 0.0f, 1.0f, 0.0f);
            break;
        case 2:
        	drawSpinner(gl2, 0.0f, 0.0f, 1.0f);
            break;
        case 3:
        	drawSpinner(gl2, 0.66f, 0.2f, 0.16f);
            break;
        case 4:
        	drawPiramid(gl2, 0.0f, 1.0f, 0.0f);
            break;
        case 5:
        	drawPiramid(gl2, 0.0f, 0.0f, 1.0f);
            break;
        case 6:
        	drawPiramid(gl2, 0.66f, 0.2f, 0.16f);
            break;
    }
	}

	private void triangle(float i, float n, GL2 gl2) 
	{
        gl2.glBegin(GL2.GL_TRIANGLES);	//primitive type is a set of triangles
        // evenly distribute the vertices around a circle of radius 1
        gl2.glVertex3d((Math.cos(i * 2 * Math.PI / n)), (Math.sin(i * 2 * Math.PI / n)), 1.68); 	
        gl2.glVertex3d((Math.cos((i+1) * 2 * Math.PI / n)), (Math.sin((i+1) * 2 * Math.PI / n)), 1.68);
        gl2.glVertex3d(0, 0, 0);
        gl2.glEnd();
    }
	
	private void drawPiramid(GL2 gl2, float arg0 , float arg1, float arg2)
	{
        gl2.glColor3f(arg0, arg1, arg2);
		gl2.glScalef(5,5,5);
        ...
        {
        	triangle(i, numberOfSides, gl2);
        }
	}
	
	private void drawSpinner(GL2 gl2, float arg0 , float arg1, float arg2) 
	{
        double n = 11;	//number of points around the curve 
        double t = 5;	//total length of the curve
        double radius = 1.5;
        double angle, nextAngle, x, y, z, h;

        h = t / 1650; 	//height of each point 
        y = -5;			//starting height
        angle = 0;		//starting angle
        gl2.glLineWidth(1);
        gl2.glColor3f(arg0, arg1, arg2);
        
        gl2.glBegin(GL.GL_LINE_STRIP);	//set of connected line segments that will form the curve
        while (y <= t) 
        {
            nextAngle = (angle * Math.PI / 180);
